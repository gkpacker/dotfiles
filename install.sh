...
if [[ ! `uname` =~ "darwin" ]]; then
  git config --global core.editor "subl3 -n -w $@ >/dev/null 2>&1"
  echo 'export BUNDLER_EDITOR="subl3 $@ >/dev/null 2>&1 -a"' >> zshrc
else
  git config --global core.editor "'/Applications/Sublime Text.app/Contents/SharedSupport/bin/subl3' -n -w"
  bundler_editor="'/Applications/Sublime Text.app/Contents/SharedSupport/bin/subl3'"
